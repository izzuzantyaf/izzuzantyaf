...
const DarkModeSlider = () => {
  ...
  return (
    <div
      onClick={controlMode}
      className={`dark-mode-slider flex rounded-full ${
        isDarkMode ? "bg-gray-800 flex-row-reverse" : "bg-white"
      } w-24 p-1 items-center gap-2 cursor-pointer`}
    >
      <div
        className={`button w-8 h-8 ${
          isDarkMode ? "bg-gray-500 flex-row-reverse" : "bg-gray-300"
        } rounded-full`}
      ></div>
      <div className={`status capitalize ${isDarkMode ? "text-white" : ""}`}>
...
export default function Navbar() {
  ...
  return (
    <div
      className={`navbar dark:bg-gray-900 fixed left-0 right-0 z-10  min-h-0 py-4 px-6 transition duration-500 ease-in-out transform ${
        isScrolledDown ? "-translate-y-20" : ""
      }`}
    >
